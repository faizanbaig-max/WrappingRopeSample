...
namespace WrappingRopeLibrary.Editors
{
    ...
    public class RopeEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            if (_rope == null)
                return;
            _isPrefab =  PrefabUtility.GetPrefabInstanceStatus(_rope) != PrefabInstanceStatus.NotAPrefab; 
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            var oldTiling = serializedObject.FindProperty("Tiling").floatValue;
            rect = GUILayoutUtility.GetLastRect();
            rect.x = 0;
            rect.y += 14;
            rect.width = EditorGUIUtility.currentViewWidth * 0.35f + 14f;
            ...
            {
                serializedObject.FindProperty("Tiling").floatValue = newTiling;
                //DoWidthChange(width);
            }
            ...
            var newMaxCount
                = UpdateParameter(
                    "Max Piece Count"
                    , serializedObject.FindProperty("MaxPieceCount").intValue
                    , (v) => { return EditorGUILayout.IntField(v); }
                    , false);
            ...
            serializedObject.FindProperty("MaxPieceCount").intValue = newMaxCount;

            serializedObject.FindProperty("NotBendOnObject").boolValue
                = UpdateParameter(
                    "Not Bend On Object"
                    , serializedObject.FindProperty("NotBendOnObject").boolValue
                    , (v) => { return EditorGUILayout.Toggle(v); }
                    , false);
        ...
        private T UpdateParameter<T>(string label, T value, Func<T, T> draw, bool light)
        {
            ...
            EditorGUILayout.LabelField(label, myStyle, GUILayout.Width(width * 0.35f));
            value = draw(value);
        ...
        private void DoBodyTypeChange(int bodyType)
        {
            ...
            {
                InitProfile();
                // May be should synchronized with other fields _baseProfile, _prevVect, _rotation
        ...
        protected virtual void ShowMeshConfiguration()
        {
            ...
            if (meshConfig == null)
                return;
            EditorGUILayout.PropertyField(meshConfig, false);
        ...
        protected virtual void ShowProfilePreview(SerializedProperty profile)
        {
            if (!profile.isArray)
                return;
            EditorGUILayout.PropertyField(profile, false);
